...
export default function Home() {
  ...
  const handleVote = (id, type) => {
    setVotes((prev) => {
      const prevCount = prev[id] || { up: 0, down: 0 };
      return {
        ...prev,
        [id]: {
          up: type === "up" ? prevCount.up + 1 : prevCount.up,
          down: type === "down" ? prevCount.down + 1 : prevCount.down,
        },
      };
    });
  };
  ...
  const handlePlaceBid = (id) => {
    ...
    if (amount && !isNaN(amount)) {
      alert(`Bid of ${amount} ETH placed on meme #${id}`);
    }
  };
          ...
          {dummyMemes.map((meme) => (
            ...
            >
              ...
              <p className="text-md text-[#39ff14] mt-2">
                💰 Highest Bid:{" "}
                <span className="text-[#00ffff]">{meme.highest_bid} ETH</span>
              </p>
